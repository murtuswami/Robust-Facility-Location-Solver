...
class LocalSearchTypeB:
    ...
    def neighbours(self,cur):
        ...
        for y in mchosen:
            for x in cur:
                neigh = copy.copy(cur)
                neigh[y] = random.randrange(0,len(self.facilsOpenCost))
...
instance = LocalSearchTypeB(initsol,customernumber,openingcosts,distances)
instance.run()
